...
function RoleCard({ role, isMobile, delay = 0, cardIndex = 0 }) {
  const [open, setOpen] = useState(!!role.open);
  const [ref, visible] = useInView(0.08);
  return (
    <div ref={ref} style={{ ...CARD,marginBottom:7,
    background:open?"radial-gradient(ellipse 65% 55% at 12% 0%, rgba(255,255,255,.1) 0%, transparent 65%), radial-gradient(ellipse 55% 45% at 88% 100%, rgba(255,255,255,.06) 0%, transparent 60%)":"radial-gradient(ellipse 65% 55% at 12% 0%, rgba(255,255,255,.08) 0%, transparent 65%), radial-gradient(ellipse 55% 45% at 88% 100%, rgba(255,255,255,.05) 0%, transparent 60%)",
    borderColor:open?"rgba(255,255,255,.16)":"rgba(255,255,255,.1)",
    boxShadow:open?"0 4px 6px rgba(0,0,0,.22),0 16px 36px rgba(0,0,0,.5),0 28px 52px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.24)":"0 4px 6px rgba(0,0,0,.2),0 12px 32px rgba(0,0,0,.45),0 24px 48px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.2)",
    ...
    transitionTimingFunction:"ease,cubic-bezier(.34,1.2,.64,1),ease,ease,ease" }}>
      ...
      {!open && role.badge && (
        <div style={{ padding:isMobile?"0 16px 10px 40px":"0 22px 12px 48px",position:"relative",zIndex:3 }}>
          <div style={{ display:"flex",gap:10,alignItems:"flex-start" }}>
            <div style={{ width:4,height:4,borderRadius:"50%",flexShrink:0,marginTop:6,background:T.blue,boxShadow:`0 0 5px ${T.blueG}` }} />
            <span style={{ fontSize:isMobile?11:11.5,fontWeight:300,color:"rgba(255,255,255,.45)",lineHeight:1.55 }}>
              {role.groups[0].bullets[0][0]}<span style={{ color:"rgba(255,255,255,.3)" }}>{role.groups[0].bullets[0][1]}</span>
            </span>
          </div>
        </div>
      )}
...
function BuildCard({ b, bi = 0, isMobile, delay = 0, enterClass = "" }) {
  ...
  const [ref, visible] = useInView(0.08);
  return (
    <div ref={ref} className={enterClass} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ ...CARD,display:"flex",flexDirection:"column",opacity:visible?1:0,transform:visible?"none":"translateY(28px)",transitionProperty:"opacity,transform,background,border-color,box-shadow",transitionDuration:`0.5s,0.5s,.28s,.28s,.28s`,transitionDelay:`${delay}ms,${delay}ms,0ms,0ms,0ms`,transitionTimingFunction:"ease,cubic-bezier(.34,1.2,.64,1),ease,ease,ease",
        background:hov
        ...
        transform:hov?"translateY(-3px)":"none",transition:"all .25s ease" }}>
      <Sheen color={b.color} />
...
function CompAccordion({ cat, defaultOpen, isMobile }) {
  ...
  return (
    <div style={{ borderRadius:8,overflow:"hidden",border:open?"1px solid rgba(255,255,255,.16)":"1px solid rgba(255,255,255,.08)",transition:"border-color .2s",animation:"cardEnter .4s ease both" }}>
      <div onClick={() => setOpen(!open)} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:isMobile?"9px 13px":"10px 14px",cursor:"pointer",background:open?"rgba(255,255,255,.055)":"rgba(255,255,255,.028)",transition:"background .18s" }}>
        <span style={{ fontSize:7.5,fontWeight:500,letterSpacing:".13em",textTransform:"uppercase",color:open?"rgba(255,255,255,.55)":"rgba(255,255,255,.32)",transition:"color .18s" }}>{cat.label}</span>
...
function SlidingToggle({ panel, setPanel, setCompOpen, isMobile }) {
  ...
  return (
    <div style={{ position:"relative",display:"flex",gap:0,padding:2,borderRadius:100,
      ...
      boxShadow:"0 8px 32px rgba(0,0,0,.4), inset 0 1.5px 0 rgba(255,255,255,.55), inset 0 -1px 0 rgba(0,0,0,.25), inset 1px 0 0 rgba(255,255,255,.08), inset -1px 0 0 rgba(255,255,255,.08)",
    }}>
      <div style={{
        position:"absolute",top:2,bottom:2,
        left: salesActive ? 2 : "calc(50% + 1px)",
        width:"calc(50% - 3px)",
        background:"rgba(255,255,255,.12)",
        borderRadius:100,
        border:"1px solid rgba(255,255,255,.2)",
        boxShadow:"0 2px 10px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.22),inset 0 -1px 0 rgba(0,0,0,.08)",
        transition:"left .32s cubic-bezier(.4,0,.2,1)",
        pointerEvents:"none",zIndex:1,
      }} />
      {["sales","builder"].map(p => (
        <button key={p} onClick={() => { setPanel(p); setCompOpen(false); }} style={{
          fontFamily:"inherit",fontSize:isMobile?9:10,fontWeight:500,letterSpacing:".1em",textTransform:"uppercase",
          color:panel===p?"rgba(255,255,255,.96)":"rgba(255,255,255,.3)",
          background:"none",border:"none",cursor:"pointer",
          padding:isMobile?"5px 16px":"5px 22px",borderRadius:100,
          position:"relative",zIndex:2,transition:"color .32s ease",
        }}>{p}</button>
      ))}
    </div>
  );
...
const ROLES = [
  { id:"r1",title:"Enterprise Account Executive",meta:"May 2023 – Present · L4 → L6",badge:"Current",open:true,
    groups:[
      { label:"Performance", bullets:[
        ...
        ["Generated 2-3 net-new enterprise opportunities per week"," through a cultivated partner network spanning NYC private equity relationships, commerce events, and co-marketing campaigns with Cash App and Afterpay. In an environment where inbound enterprise leads average one every two months."],
      ...
      { label:"Key Achievements", bullets:[
        ["Directly called out by Square's board of directors"," as the organizational benchmark for enterprise execution, the first individual AE whose methodology was elevated to board-level recognition."],
        ["Closed an 18-month enterprise deal"," with a national commercial real estate developer, navigating a 20+ stakeholder buying committee across Legal, Finance, and board review, then managing a 5,000+ person implementation. Currently in active onboarding."],
        ["Promoted to Enterprise AE in 15 months"," through a path that didn't exist, the first BDR-to-Enterprise-AE transition in Square's history, requiring a custom career track to be built specifically for the move."],
      ]},
    ]},
  { id:"r2",title:"Enterprise & Strategic Services BDR",meta:"February 2022 – May 2023 · L3",
    groups:[
      { label:"Performance", bullets:[
        ["Averaged 190% quota attainment Q1-Q4"," sourcing enterprise pipeline across Healthcare, Hospitality, and QSR, with BDR-originated opportunities consistently outperforming team close rates."],
        ["Built Square's first channel sales motion from scratch"," outbounding to private equity and consulting firms and growing closed-won opportunities 40% YoY with higher intent and shorter sales cycles."],
      ]},
      { label:"Key Achievement", bullets:[
        ["Promoted to Enterprise AE in 15 months"," through a path that didn't exist, the first BDR-to-Enterprise-AE transition in Square's history, requiring a custom career track to be built specifically for the move."],
      ]},
    ]},
  { id:"r3",title:"Account Executive, SMB",meta:"May 2021 – February 2022 · L2",
    groups:[
      { label:"Performance", bullets:[
        ["Averaged 178% quota attainment"," (Q1: 175%, Q2: 195%, Q3: 165%) running full-cycle deals across larger-than-typical accounts with multi-stakeholder buying processes, demonstrating enterprise-tier execution within an SMB role."],
        ["Identified and developed the SuiteRetail partnership"," now a preferred Square Partner, building a referral network of 20+ midmarket and enterprise sellers generating a consistent pipeline of one qualified lead per week."],
        ["Selected as the only SMB AE at Square"," to partner with the Enterprise AE on the Cardinal Health account, running an enterprise-style channel sales motion, multi-threading across 10+ pharmacy and Cardinal Health stakeholders to build and scale the partnership from the ground up."],
      ]},
      { label:"Key Achievement", bullets:[["Promoted to Enterprise & Strategic Services BDR in 9 months"," the fastest transition of its kind in Square's history, recognized for consistently running enterprise-level deal complexity within an SMB book."]] },
    ]},
  { id:"r4",title:"SMB Business Development Representative",meta:"August 2020 – May 2021 · L1",
    groups:[
      { label:"Performance", bullets:[
        ["Averaged 182% quota attainment Q1-Q4"," consistently recognized among the top BDR performers in the organization across outbound activity, demo-set volume, and conversion rate."],
        ["Executed targeted outbound"," across Restaurant, Retail, and Healthcare verticals from day one, developing the account-level targeting approach that carried into every subsequent role at Square."],
      ]},
  ...
  { id:"r5",title:"Marketing & Graphics Associate",meta:"2018 – 2020",
    groups:[
      { label:"Experience", bullets:[
        ["Sales collateral and full-motion imagery"," that improved inbound lead quality and shortened prospect evaluation timelines."],
...
const BUILDS = [
  { color:"#8ba4c8",name:"Enterprise Intelligence",problem:"CRM required hours of manual research weekly, no reliable way to surface PE investments, leadership changes, or funding signals at scale.",desc:"Account intelligence dashboards for key enterprise targets, auto-surfacing private equity investments, leadership mapping, partner connections, and buying signals like recent funding rounds.",tools:["Claude","Glean","MCP"],metrics:["Hours/week recovered","Full account maps pre-call"] },
  { color:"#a8b8cc",name:"Prospecting Websites",problem:"Standard outbound sequences couldn't cut through noise in a competitive NYC enterprise market at scale.",desc:"Personalized prospect websites with custom branding and account-specific value messaging, showing exactly what Square uniquely delivers to each target, replacing generic outreach entirely.",tools:["Claude","MCP"],metrics:["8/15 NYC prospects responded","Within 3 weeks"] },
  { color:"#9d8fc4",name:"Deal Brain",problem:"30+ live deals, no single source of truth, context fragmenting across channels and calls.",desc:"Live auto-updating repository ingesting transcripts, Slack, email, and SMS via BlueBubble API. Maintains structured deal files mirroring a Notion project system, accessible from mobile for real-time field use.",tools:["Claude","Goose","Glean","GitHub","BlueBubble API"],metrics:["35% faster deal prep","Hourly sync"] },
  { color:"#7a9bbf",name:"Ask Panels",problem:"Generic AI lacked deal context to answer accurately during live enterprise meetings.",desc:"Per-deal assistant drawing from Deal Brain's source of truth: product timeline, stakeholder history, and org intelligence. Credential-gated by org-chart for sensitive sharing.",tools:["Claude","Goose","Glean","MCP","Revenue.io"],metrics:["~70% faster live responses"] },
  ...
  { color:"#c4a882",name:"Live Demo Engine",problem:"Static decks couldn't reflect account-specific data or respond to live questions during enterprise meetings.",desc:"Built and deployed custom Square x Claude MCP dashboards live during prospect meetings, real-time account-specific tools created on the spot, replacing prepared decks entirely.",tools:["Square","Claude","MCP"],metrics:["45% higher follow-up rate","25% higher deal values"] },
  { color:"#b89a52",name:"PM Onboarding Flow",problem:"Multi-concept merchants with 40+ implementation stakeholders break every standard onboarding flow.",desc:"Multi-threaded system with pre-mortem risk analysis, auto-generated CPQ orders (100+ per deal) via Claude, and custom charts, Q&A forms, and sentiment tracking per channel and franchise group.",tools:["Claude","Glean","MCP","CPQ automation"],metrics:["30% faster implementation","100+ CPQ orders automated","40+ stakeholders managed"] },
];
...
export default function App() {
  ...
  return (
    <div style={{ background:T.bg,minHeight:"100vh",fontFamily:"Jost,system-ui,sans-serif",color:T.txt,position:"relative" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        @keyframes bounce{0%,100%{transform:translateY(0);opacity:.35}50%{transform:translateY(7px);opacity:.8}}
        @keyframes fadeUp{from{opacity:0;transform:translateY(18px)}to{opacity:1;transform:none}}
        @keyframes fadeInBg{from{opacity:0}to{opacity:1}}
        @keyframes slideDown{from{transform:translateX(-50%) translateY(-18px) scale(.96);opacity:0}to{transform:translateX(-50%) translateY(0) scale(1);opacity:1}}
        ...
        .fu2{animation:fadeUp .65s .14s ease both}
        .fu3{animation:fadeUp .65s .28s ease both}
        @keyframes nameReveal{0%{letter-spacing:.12em;opacity:0;filter:blur(6px)}100%{letter-spacing:-.025em;opacity:1;filter:blur(0)}}
        .name-reveal{animation:nameReveal .9s cubic-bezier(.4,0,.2,1) both}
        @keyframes cardEnter{0%{opacity:0;transform:translateY(24px) scale(.98)}100%{opacity:1;transform:none}}
        .card-enter-1{animation:cardEnter .55s .05s cubic-bezier(.34,1.1,.64,1) both}
        .card-enter-2{animation:cardEnter .55s .13s cubic-bezier(.34,1.1,.64,1) both}
        .card-enter-3{animation:cardEnter .55s .21s cubic-bezier(.34,1.1,.64,1) both}
        .card-enter-4{animation:cardEnter .55s .29s cubic-bezier(.34,1.1,.64,1) both}
        .card-enter-5{animation:cardEnter .55s .37s cubic-bezier(.34,1.1,.64,1) both}
        .card-enter-6{animation:cardEnter .55s .45s cubic-bezier(.34,1.1,.64,1) both}
        .card-enter-7{animation:cardEnter .55s .53s cubic-bezier(.34,1.1,.64,1) both}
        .fu4{animation:fadeUp .8s .72s ease both}
      ...
      <div style={{ position:"fixed",top:0,left:0,right:0,zIndex:100,height:isMobile?48:52,display:"flex",alignItems:"center",justifyContent:"space-between",padding:isMobile?"0 16px":"0 32px",background:"rgba(10,11,16,.78)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,255,255,.1)",boxShadow:"0 1px 0 rgba(255,255,255,.07),0 8px 24px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.08)" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:8 }}>
          ...
          <div style={{ borderRadius:100,background:compOpen?"rgba(255,255,255,.1)":"rgba(255,255,255,.05)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",border:compOpen?"1px solid rgba(255,255,255,.22)":"1px solid rgba(255,255,255,.14)",boxShadow:compOpen?"0 2px 10px rgba(0,0,0,.25),inset 0 1px 0 rgba(255,255,255,.2),inset 0 -1px 0 rgba(0,0,0,.08)":"0 3px 12px rgba(0,0,0,.2),inset 0 1px 0 rgba(255,255,255,.14),inset 0 -1px 0 rgba(0,0,0,.08)",transition:"all .28s cubic-bezier(.4,0,.2,1)" }}>
        ...
        <>
          ...
          <div style={{ position:"fixed",top:isMobile?60:68,left:"50%",width:isMobile?"calc(100vw - 32px)":"580px",maxHeight:"70vh",zIndex:90,animation:"slideDown .28s cubic-bezier(.34,1.56,.64,1) forwards" }}>
            <div style={{ background:"rgba(13,14,19,.22)",backdropFilter:"blur(48px)",WebkitBackdropFilter:"blur(48px)",border:"1px solid rgba(255,255,255,.1)",borderRadius:20,boxShadow:"0 16px 40px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.1)",overflow:"hidden",maxHeight:"70vh",overflowY:"auto" }}>
              <div style={{ position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent 5%,rgba(255,255,255,.18) 25%,rgba(255,255,255,.28) 50%,rgba(255,255,255,.18) 75%,transparent 95%)",zIndex:2,pointerEvents:"none" }} />
      ...
      <div style={{ position:"relative",zIndex:1,paddingTop:isMobile?48:52 }}>
        {panel === "sales" && (
          <div>
            <div style={{ minHeight:isMobile?"calc(100svh - 48px)":"calc(100vh - 52px)",display:"flex",flexDirection:"column",justifyContent:"flex-start",padding:isMobile?"44px 20px 0":"52px 44px 0",maxWidth:1060,margin:"0 auto",position:"relative" }}>
              <div className="fu">
                ...
                <div style={{ fontSize:isMobile?8.5:9.5,fontWeight:400,letterSpacing:".16em",textTransform:"uppercase",color:"rgba(255,255,255,.35)",marginBottom:isMobile?12:16 }}>
                  Enterprise Account Executive · Square · New York City
                </div>
                <div style={{ fontSize:isMobile?13:15.5,fontWeight:300,color:T.txt2,lineHeight:1.78,maxWidth:600,marginBottom:10 }}>
                  Six years at Square across four roles,{" "}
                  <strong style={{ color:T.txt,fontWeight:400 }}>ranked #1 AE globally</strong>.
                  {" "}The path that got there was unconventional. Self-taught designer, marketing background, no traditional sales pedigree. Came into Square as an SMB BDR and ran every IC sales role on the way up. That foundation turned out to be an asset, not a detour.
                  More recently, building AI agents and tools from real operational problems my team and I are running into every day.
                </div>
                <div className="fu3" style={{ fontSize:isMobile?14:16,fontWeight:300,color:"rgba(255,255,255,.85)",lineHeight:1.5,marginBottom:isMobile?14:18 }}>
                  What excites me most is where sales is heading, the future sits at the intersection of{" "}
                  <strong style={{ color:T.txt,fontWeight:500 }}>Sales x Design x Automation</strong>.
                </div>
                <div style={{ display:"flex",alignItems:"center",gap:9,fontSize:isMobile?10:10.5,color:"rgba(45,212,170,.78)" }}>
                  ...
                  {isMobile?"Open to conversations · 917.608.6653":"Open to conversations · 917.608.6653 · linkedin.com/in/josephamari"}
                </div>
              ...
              <div className="fu4" style={{ display:"flex",flexDirection:"column",gap:isMobile?10:12,marginTop:isMobile?"5vh":"8vh" }}>
                <div style={{ width:"100%",background:"rgba(255,255,255,.04)",border:"1px solid rgba(255,255,255,.12)",borderRadius:16,overflow:"hidden",boxShadow:"0 8px 24px rgba(0,0,0,.3),inset 0 1px 0 rgba(255,255,255,.2)",position:"relative" }}>
                  ...
                  <div style={{ position:"absolute",top:0,bottom:0,width:"30%",background:"linear-gradient(90deg,transparent,rgba(255,255,255,.06),transparent)",animation:"sweep 1.8s 1.1s ease forwards",zIndex:1,pointerEvents:"none" }} />
                  <div style={{ display:"flex",width:"100%",position:"relative",zIndex:3 }}>
                    {STATS.map((s,i) => (
                      <div key={s.v} style={{ flex:1,padding:isMobile?"10px 0":"12px 0",display:"flex",flexDirection:"column",alignItems:"center",gap:isMobile?1:3,borderLeft:i>0?"1px solid rgba(255,255,255,.09)":"none" }}>
                        ...
                        <span style={{ fontSize:isMobile?6.5:8.5,fontWeight:300,color:"rgba(255,255,255,.38)",letterSpacing:".05em",textTransform:"uppercase",textAlign:"center",padding:isMobile?"0 2px":0 }}>{s.l}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ width:"100%",background:"rgba(255,255,255,.03)",border:"1px solid rgba(255,255,255,.1)",borderRadius:16,overflow:"hidden",boxShadow:"0 8px 24px rgba(0,0,0,.28),inset 0 1px 0 rgba(255,255,255,.16)",position:"relative",padding:isMobile?"10px 12px":"11px 16px" }}>
                  <div style={{ position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,rgba(255,255,255,.18) 30%,rgba(255,255,255,.24) 50%,rgba(255,255,255,.18) 70%,transparent)",zIndex:2 }} />
                  <div style={{ position:"absolute",top:0,bottom:0,width:"30%",background:"linear-gradient(90deg,transparent,rgba(255,255,255,.04),transparent)",animation:"sweep 1.8s 1.4s ease forwards",zIndex:1,pointerEvents:"none" }} />
                  <div style={{ display:"flex",alignItems:"center",flexWrap:"wrap",gap:isMobile?"5px 8px":"5px 10px",position:"relative",zIndex:3 }}>
                    <span style={{ fontSize:isMobile?6.5:7.5,fontWeight:500,letterSpacing:".12em",textTransform:"uppercase",color:"rgba(255,255,255,.28)",marginRight:4,flexShrink:0,whiteSpace:"nowrap" }}>Verticals</span>
              ...
              <div style={{ position:"absolute",bottom:0,left:0,right:0,height:`${(isMobile?90:110) + Math.min(scrollY * 0.55, isMobile?200:260)}px`,background:"linear-gradient(to bottom,transparent 0%,rgba(13,14,19,.75) 35%,#0d0e13 70%)",pointerEvents:"none",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",paddingBottom:isMobile?18:22,zIndex:2 }}>
              ...
              <div style={{ position:"absolute",bottom:0,left:0,right:0,zIndex:3 }}>
                ...
                <div style={{ position:"absolute",top:1,left:0,right:0,height:32,background:"linear-gradient(to bottom,rgba(0,0,0,.45) 0%,rgba(0,0,0,.18) 40%,transparent 100%)",pointerEvents:"none" }} />
              </div>
            </div>
            ...
            <div style={{ maxWidth:1060,margin:"0 auto",padding:`0 ${pad} ${isMobile?"48px":"80px"}` }}>
              ...
              {[["Square","August 2020 – Present · New York, NY",0,4],["Tzumi Electronics","2018 – 2020 · New York, NY",4,5]].map(([name,dates,from,to]) => (
                <div key={name}>
                  ...
                  {ROLES.slice(from,to).map((r,ri) => <RoleCard key={r.id} role={r} isMobile={isMobile} delay={ri*80} cardIndex={from+ri} />)}
                </div>
              ))}
            </div>
            </RevealSection>
        ...
        {panel === "builder" && (
          <div key="builder-panel">
            <div style={{ padding:isMobile?"40px 20px 28px":"56px 44px 40px",maxWidth:1060,margin:"0 auto" }}>
              <div className="name-reveal" style={{ fontFamily:"Georgia,'Times New Roman',serif",fontSize:isMobile?36:58,fontWeight:300,lineHeight:1,marginBottom:isMobile?10:18,background:"linear-gradient(175deg,rgba(255,255,255,1) 0%,rgba(255,255,255,.68) 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text" }}>
                Builder, <em>in the field.</em>
              </div>
              <div className="fu" style={{ fontSize:isMobile?8:9.5,fontWeight:400,letterSpacing:".16em",textTransform:"uppercase",color:"rgba(255,255,255,.35)",marginBottom:isMobile?10:14 }}>Not an engineer. A seller who builds from real pain.</div>
              <div className="fu3" style={{ fontSize:isMobile?12.5:14,fontWeight:300,color:T.txt2,lineHeight:1.82,maxWidth:580 }}>
                Every system below was built live, with <strong style={{ color:T.txt,fontWeight:400 }}>real accounts on the line</strong>, because the tooling broke down at the complexity of 30+ concurrent enterprise deals.
                {" "}Each one was designed to be scalable, built to solve problems I was facing personally, then scoped and structured so they could be deployed across Square's broader sales organization.{" "}
                <strong style={{ color:T.txt,fontWeight:400 }}>All are currently in use or in active rollout across the team.</strong>
              </div>
            </div>
